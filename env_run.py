...
class Env:
    def __init__(self):
        ...
        self.user_perenb = 12
        self.user_number = self.user_perenb * len(enb_list)
        self.userlist = 0
        self.request_list = 0
        ...
        self.RbgMap = np.zeros((len(enb_list), self.rbgnumber))#资源块的占用情况，初始化为基站数*资源块数目,0表示未占用
        self.InvFlag = np.random.randint(1, 2, size=(len(enb_list), self.user_number))#有效用户数，1表示无效，0是有效的，维度是基站数*用户数
        ...
        self.observation_space = {'Requests': (self.user_number, 22), 'RbgMap': (len(enb_list), self.rbgnumber),
                                  'InvFlag': (len(enb_list), self.user_number)}#资源块的使用情况，还有就是每个基站下用户是否有效
        self.action_space = (self.rbgnumber * len(enb_list), self.user_number + 1)#动作空间就是系统下的资源个数*用户数，也就是给每个用户分配多个资源，或者就是对于每个资源来说应该分给那个用户或者部分
        self.extra_infor = {}
    ...
    def reset(self, on, off):
        ...
        # 初始化所有用户位置
        self.userlist = initial_all_user(self.enb_radius, self.user_perenb,enb_list, ontime=on, offtime=off)
        # 初始化所有用户业务
        ...
        # 根据最近距离寻找接入基站ag
        enb_user_info = user_connect_enb(position_xyz0)# 返回的是一个用户坐标，和接入基站的坐标和基站的编号所组成的列表，列表中每个元素是一个元组 UE HE ENB
    ...
    def step(self, action=0):
        ...
        enb_user_info = user_connect_enb(position_xyz)
    ...
    def generate_InvFlag(self, data):
        flag = np.random.randint(1, 2, size=(len(enb_list), self.user_number))
        for i in range(len(enb_list)):
            b = np.where(data == i + 1)
            flag[i][b] = 0
        return flag
...
def user_connect_enb(position):
    position_list = position
    enb_number_user_position_list = []
    for i in range(len(position_list)):
        userposition_and_enb_position = caculate_distance(position_list[i])
        enb_number_user_position_list.append(userposition_and_enb_position)
    return enb_number_user_position_list


# 最近距离选取接入基站
def caculate_distance(position):
    distance_list = []
    for i in range(len(enb_list)):
        distance = math.sqrt(
            math.pow((position[0] - enb_list[i][0]), 2) + math.pow((position[1] - enb_list[i][1]), 2) + math.pow(
                (position[2] - enb_list[i][2]), 2))
        distance_list.append(distance)
    enb_connect = distance_list.index(min(distance_list))
    label=calc_radian(position,enb_list[enb_connect])
    return (position, enb_list[enb_connect], enb_connect + 1,distance_list[enb_connect],label)

# 计算平均吞吐量和公平性
def calc_radian(ue,enb):
